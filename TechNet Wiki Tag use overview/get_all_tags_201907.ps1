...
foreach ($url in $sitemap.urlset.url.loc)
{
    ...
    if ($url.Contains("/tags/"))
    {
...

        {
            ...
            #$articleInfo | Export-Excel $xlsFile -Append
...
#Check file and count tags saved
...
Write-host "Total count: " $TagTotalCount
...
"The script took {0:g} to run." -f (NEW-TIMESPAN –Start $StartTime –End $EndTime)
